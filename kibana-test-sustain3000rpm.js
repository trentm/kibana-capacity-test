 ...
 */
function runIntervalsForDuration(rpm, intervalLenMins, configObject) {
    // target requests per second
    const rps = rpm / 60;
    // interval duration in seconds
    const intervalDurationSec = REQUESTS_PER_BATCH / rps;
    // Overall number of requests to run
    const requestsToRun = intervalLenMins * rpm;
...
async function initTest(testName = 'login') {
    ...
    await runIntervalsForDuration(100, 0.5, testData);

    do {
        try {
            console.log(`Running test for ${rpm} rpm, interval duration is ${INTERVAL_LENGTH_MINS} mins`)
            latestBenchMark = benchmarks[rpm] = await runIntervalsForDuration(rpm, INTERVAL_LENGTH_MINS, testData);
